...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val panelBg = Color.parseColor("#BF101418")
        ...
        // Start leveling / Live overlay / Mic — stacked, right-locked, matching width.
        ...
        right.addView(actionButtons)

        topView = text("", 18f).apply { setPadding(0, dp(8), 0, dp(4)) }
        right.addView(topView)
     ...
     *  Uses adjustStreamVolume() one step at a time — same as LevelerService.step() —
     *  rather than a single setStreamVolume() jump, which this HDMI-CEC/ARC output
     *  doesn't reliably keep. */
    private fun maybeBoostVolumeOnLock() {
        ...
        var cur = am.getStreamVolume(AudioManager.STREAM_MUSIC)
        while (cur < target) {
            am.adjustStreamVolume(AudioManager.STREAM_MUSIC, AudioManager.ADJUST_RAISE, 0)
            val next = am.getStreamVolume(AudioManager.STREAM_MUSIC)
            if (next <= cur) break // volume is fixed or stuck; stop rather than loop forever
            cur = next
        }
    ...
    private fun refresh() {
        ...
        val overlayLabel = when {
            !Prefs.overlayOn(this) -> "Off"
            granted -> "On"
            else -> "needs permission — press Enable overlay to grant"
        }
